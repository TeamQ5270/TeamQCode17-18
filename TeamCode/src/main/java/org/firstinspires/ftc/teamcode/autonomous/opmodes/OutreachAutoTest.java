...
public class OutreachAutoTest extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while(opModeIsActive()&&MultiMotor.busyMotors(motors)) {
            telemetry.addData("at ", leftMotor.getCurrentPosition());
            telemetry.addData("moving to ", leftMotor.getTargetPosition());
            telemetry.update();
        }
        ...
        while (!MultiMotor.turnBetter(leftMotors, rightMotors, 90, gyro.getHeading(), 0.005f, 1)&&opModeIsActive()) {
            telemetry.addData("gyro: ", gyro.getHeading()); //Turn using the passed in values, and +-tolerance degree
            telemetry.update();
        }
